...
float sdPlane2(float3 p, float4 n)
{
...
//mergerSponge
float2 sdMenger( in float3 p, float b, int _iterations, float3 _modOffsetPos ,  float _smoothRadius, float _scaleFactor, float4x4 _iterationTransform)
{
...
//mengerSponge cylinder
float2 sdMengerCyl( in float3 p, float b, int _iterations, float3 _modOffsetPos, float _smoothRadius, float _scaleFactor, float4x4 _iterationTransform) //, float4x4 _iterationTransform
{
   //p = mul(_globalTransform, float4(p,1)).xyz;
...
float2 sdMengerPyr( in float3 p, float b, int _iterations, float3 _modOffsetPos , float _smoothRadius, float _scaleFactor, float4x4 _iterationTransform)
{
   b = 2*b;
   //p = mul(_globalTransform, float4(p,1)).xyz;
...
//mandelbulb
float2 mandelbulb (in float3 p,float _power,float b,float _iterations,float _smoothRadius){
